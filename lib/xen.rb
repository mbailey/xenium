...
class XenDomain
  ...
  def initialize(name)
    @name = name
    @config = XenConfig.find(name)
    @instance = XenInstance.find(name)
    @image = XenImage.find(name)
  end
  
  def self.find(*args)
    options = args.extract_options!
    case args.first
      when :all       then XenConfig.find(:all, options).collect { |config| XenDomain.new(config.name) }
      when :running   then XenInstance.find(:all, options).collect { |instance| XenDomain.new(instance.name) }
      # Retrieve a Domain by name
      else            XenConfig.find_by_name(args.first) && self.new(args.first)
    end
  end
